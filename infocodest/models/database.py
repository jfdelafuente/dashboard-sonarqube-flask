...
def ejecutar_consultas(conn, queries, params):
    results = {
        key: conn.execute(text(query), params).fetchone()
...
def calcular_datos(datos, old_datos, keys):
    ...
    for key in keys:
        ...
        resultados[f"{key}_text"] = "{:.2f}% {} in {} Days".format(
            avance, comparativa, DAYS
        )
...
def getDatosMetricas():
    ...
    return calcular_datos(
        datos,
        old_datos,
        ["aplicaciones", "repositorios", "bugs", "analisis", "quality"],
    )
