...
def historico_project(aplicacion):
    data = {}
    metricas = (
        Metrica.query.filter(Metrica.aplicacion == aplicacion)
...
def historico_name(project, name):
    data = {}
    scores = (
        Historico.query.filter(Historico.repo == name, Historico.aplicacion == project)
        .order_by(Historico.fecha.asc())
        .all()
    )
    ...
    return json.dumps(
        [
            ...
            for registro in Registro.query.filter(
                Registro.created_on
                >= date(
                    year=fecha_actualizada.year,
                    month=fecha_actualizada.month,
                    day=fecha_actualizada.day,
                )
            )
        ]
    )
...
def rating_historico_name(project, name):
    data = {}
    scores = (
        Metrica.query.filter(Metrica.repo == name, Metrica.aplicacion == project)
        .order_by(Metrica.fecha.asc())
        .all()
    )
    data["project_name"] = [row.repo for row in scores]
...
def daily_aplicacion(aplicacion):
    data = {}
    dailys = (
        Daily.query.filter(Daily.aplicacion == aplicacion)
...
def daily_aplicacion_metrica(aplicacion):
    ...
    data = {}
    dailys = (
        Daily.query.filter_by(aplicacion=aplicacion)
        .group_by(Daily.created_on)
        .order_by(Daily.created_on.asc())
        .with_entities(
            Daily.aplicacion, Daily.created_on, func.sum(campo).label("metrica")
        )
        .all()
    )
...
def daily_aplicacion_repo(aplicacion, repo):
    data = {}
    dailys = (
        Daily.query.filter(Daily.repo == repo, Daily.aplicacion == aplicacion)
        .group_by(Daily.created_on)
        .order_by(Daily.created_on.asc())
        .with_entities(
            Daily.aplicacion,
...
def daily_proveedor(proveedor):
    ...
    dailys = (
        Daily.query.filter(Daily.proveedor == proveedor)
        .group_by(Daily.created_on)
        .order_by(Daily.created_on.asc())
        .with_entities(
            Daily.proveedor,
